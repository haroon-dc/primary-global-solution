...
class WP_Theme_Custom {
	...
	public static function defaults( $item_id = null ) {
		if ( $item_id ) {
			...
			if ( self::is_blog() ) {
				$post_id = self::page_by_template( 'blog' );
			}
		}
	 ...
	 * @return void
	 */
	public static function the_attachment_image( $image_id, $thumb_size = 800, $args = array(), $hide_backup = false ) {
		$alt                    = get_post_meta( $image_id, '_wp_attachment_image_alt', true );
		$title                  = get_the_title( $image_id );
		$thumb_size_with_prefix = 'thumb_' . $thumb_size;
	 ...
	 * @return void
	 */
	public static function the_featured_image( $post_id, $thumb_size = 100, $args = array(), $hide_backup = false ) {
		...
		if ( $post_image_id ) {
			...
			if ( ! $hide_backup ) {
				// If there is no featured image, display the default image.
	 ...
	 */
	public static function enqueue_script( $path, $dependencies = array( 'jquery' ), $local_vars = null, $data = array() ) {
		$version = filemtime( get_template_directory() . '/' . $path );
		$name    = explode( '.', basename( $path ) )[0] ?? basename( $path );
		$name    = 'wp-theme-' . $name;
		wp_register_script( $name, get_template_directory_uri() . '/' . $path, $dependencies, $version, false );
		if ( $local_vars ) {
			wp_localize_script(
				$name,
				$local_vars,
